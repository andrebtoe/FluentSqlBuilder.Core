...
namespace FluentSqlBuilder.Playground
{
    public static class ColorsExtension
    {
        public struct ColorWrap
        {
            ...
            public ColorWrap(byte start, byte end) : this()
            {
                Start = start;
                End = end;
            }
        ...
        {
            return RunSequencer(s, (str, i, c) => (0 == i % 2) ? Wrap(c, "inverse") : c.ToString());
        }

        public static string Rainbow(this string s)
        {
            var rainbowColors = new[] { "red", "yellow", "green", "blue", "magenta" }; //RoY G BiV
            return RunSequencer(s, (str, i, c) => (char.IsWhiteSpace(c) ? c.ToString() : Wrap(c, rainbowColors[(i + 1) % rainbowColors.Length])));
        }
    }
}